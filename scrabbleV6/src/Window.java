...
private List<String> getFormedWords(Board board, List<Tile> placedTiles, List<Tile> permanentPlacedTiles) {
    ...
    for (Tile tile : placedTiles) {
        ...
        if (adjacentTileAbove != null) {
            ...
            formedWords.add(verticalWord.toString());
        }

        if (adjacentTileToLeft != null) {
            ...
            formedWords.add(horizontalWord.toString());
        }
    }

    for (int i = 0; i < formedWords.size(); i++) {
        String word = formedWords.get(i);
        formedWords.set(i, word.replaceAll("\\s+", ""));
    }
